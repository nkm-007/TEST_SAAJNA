...
import { Loader } from "@/components/loader";
import { CreateTaskDialog } from "@/components/task/create-task-dialog";
import { ProjectNameEditor } from "@/components/project/project-name-editor";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardHeader } from "@/components/ui/card";
...
const TaskCard = ({ task, onClick }: { task: Task; onClick: () => void }) => {
  return (
    <Card
      ...
      className="cursor-pointer hover:shadow-md transition-all duration-300 hover:translate-y-1"
    >
      <CardHeader className="px-3 md:px-4 pb-2">
        <div className="flex items-center justify-between">
          <Badge
            className={cn(
              "text-xs",
      ...
      <CardContent className="px-3 md:px-4">
        <h4 className="font-medium mb-2 text-sm md:text-base">{task.title}</h4>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center md:justify-between text-xs md:text-sm gap-2">
          <div className="flex items-center gap-2">
            {task.assignees && task.assignees.length > 0 && (
              <div className="flex -space-x-2">
                {task.assignees.slice(0, 5).map((member) => (
                  <Avatar
                    key={member._id}
                    className="relative size-6 md:size-8 bg-gray-700 rounded-full border-2 border-background overflow-hidden"
                    title={member.name}
                  >
                    <AvatarImage src={member.profilePicture} />
